...
`Dialog` рисуется в **отдельном окне**: содержимое за ним принадлежит окну активити — другому дереву
render node. `Modifier.blur()` внутри диалога размоет сам диалог, а не это содержимое.

Backdrop-библиотеки границу окна переходят: haze записывает поддерево, помеченное `hazeSource`, и
проигрывает запись под эффектом —
[в том числе в диалоге](https://chrisbanes.github.io/haze/latest/core-concepts/#dialogs). Для этого
фон должен быть вашим Compose-деревом, которое есть чем пометить.

Underlay — для случая, когда это не так: за окном View-иерархия или чужой код, а оверлей не
подкомпозиция экрана, а `PopupWindow`. Underlay ничего не просит у композиции и работает уровнем
ниже, на окнах: `FLAG_BLUR_BEHIND` на API 31+, ниже — снимок через `PixelCopy`. У самой платформы
ответа по-прежнему нет:
[Support window blur in compose dialogs](https://issuetracker.google.com/issues/296272625).

Совмещаются: Underlay даёт подложку из чужого окна, in-window библиотека — живой эффект поверх.
