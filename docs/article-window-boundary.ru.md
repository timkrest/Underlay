...
Дизайнер прислал макет: карточка поверх ленты, под карточкой матовое стекло. Я повесил haze, и
диалог заблюрил сам себя: содержимое карточки поплыло, а лента за ней осталась резкой.
...
забирает то, что оно нарисовало, и размывает. haze, Cloudy, imla, `Modifier.blur()` из самого
